--[[
    ...
    Features:
      ...
      • Stronger 2x structural animations
...
function Library:SetMenuVisible(visible, instant)
    ...
    local duration = math.max(0.20, tonumber(self.Settings.AnimationSpeed) or 0.18)
    ...
    if self.MenuVisible then
        ...
        if self.Settings.OpenAnimation == "Scale" then
            ...
            Main.Position = basePos
            MainScale.Scale = baseScale * 0.52
        ...
        elseif self.Settings.OpenAnimation == "Slide" then
            ...
            Main.Position = UDim2.new(
                ...
                basePos.X.Offset,
                basePos.Y.Scale,
                basePos.Y.Offset + 260
            )
            MainScale.Scale = baseScale * 0.94
            MainVisual.GroupTransparency = 0.08

            self:Animate(Main, duration * 1.08, {
                Position = basePos,
            })
        ...
        task.delay(duration * 1.12 + 0.05, function()
...
function Library:CreateTab(name, localeKey)
    ...
    function tab:CreateSection(title, defaultOpen, localeKey)
        ...
        function section:SetOpen(state, instant)
            ...
            local duration = math.max(0.14, (Library.Settings.AnimationSpeed or 0.18) * 0.95)

            if instant then
                Items.Visible = state
                Items.GroupTransparency = state and 0 or 1
                Items.Position = UDim2.fromOffset(0, 44)
                ItemsScale.Scale = state and 1 or 0.80
            ...
            if state then
                ...
                Items.Position = UDim2.fromOffset(0, 16)
                ItemsScale.Scale = 0.80
                Arrow.Text = "-"

                -- Animate the section as a whole...
                Library:Animate(Items, duration, {
                    GroupTransparency = 0,
                    Position = UDim2.fromOffset(0, 44),
                })
                Library:Animate(ItemsScale, duration, {Scale = 1})
                Library:Animate(HeaderStroke, duration, {Transparency = 0.12})

                -- ...and give each function row its own small staggered scale
                -- so toggles/sliders/text visibly participate in the motion.
                for index, control in ipairs(self.Controls) do
                    if control.MotionScale and control.MotionScale.Parent then
                        control.MotionScale.Scale = 0.88
                        task.delay(math.min((index - 1) * 0.028, 0.16), function()
                            if section.AnimationToken == token
                                and section.Open
                                and control.MotionScale
                                and control.MotionScale.Parent
                            then
                                Library:Animate(control.MotionScale, duration * 0.72, {Scale = 1})
                            end
                        end)
                    end
                end
                ...
                for _, control in ipairs(self.Controls) do
                    if control.MotionScale and control.MotionScale.Parent then
                        Library:Animate(control.MotionScale, duration * 0.65, {Scale = 0.90})
                    end
                end
                ...
                Library:Animate(ItemsScale, duration, {Scale = 0.80})
                Library:Animate(HeaderStroke, duration, {Transparency = 0.24})
...
Library:AdaptiveCorner(WatermarkAccent, -4)
...
local function finishLoader()
    ...
    -- Keep the loader on-screen for at least five seconds. Besides looking
    -- deliberate, this gives filesystem-backed autoload/config code time to
    -- settle before the actual menu becomes interactive.
    local loaderStarted = os.clock()
    local minimumLoaderTime = 5

    LoaderStatus.Text = Library:L("LoadingModules")
    TweenService:Create(LoaderFill, TweenInfo.new(0.65, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        Size = UDim2.fromScale(0.28, 1),
    }):Play()
    task.wait(1.15)

    LoaderStatus.Text = Library:L("PreparingInterface")
    TweenService:Create(LoaderFill, TweenInfo.new(0.75, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
    ...
    LoaderStatus.Text = Library:L("Finalizing")
    TweenService:Create(LoaderFill, TweenInfo.new(0.55, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
        Size = UDim2.fromScale(1, 1),
    }):Play()

    -- Reserve the final fade inside the five-second budget.
    local fadeDuration = 0.24
    ...
    TweenService:Create(LoaderBox, TweenInfo.new(fadeDuration), {
        BackgroundTransparency = 1,
    }):Play()
    TweenService:Create(Loader, TweenInfo.new(fadeDuration), {
        BackgroundTransparency = 1,
    }):Play()
