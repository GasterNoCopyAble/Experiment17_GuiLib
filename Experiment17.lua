-- Experiment 17 Legacy UI Library stable loader
-- Current implementation: Legacy v22
...
if not ok or type(source) ~= "string" or #source == 0 then
    error("[Experiment17] Failed to download Legacy v22 core: " .. tostring(source))
end

local chunk, compileError = compiler(source)
if not chunk then
    error("[Experiment17] Failed to compile Legacy v22 core: " .. tostring(compileError))
end
